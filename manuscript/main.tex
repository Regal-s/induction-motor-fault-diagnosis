...
and, crucially, with \emph{Leave-One-Load-Out} (LOLO) cross-validation. We show that
...
whereas absolute severity is load-dependent; a SHAP-guided analysis isolates the cause and a
load-aware reformulation restores cross-load severity (within-one-level accuracy
$0.73\rightarrow0.95$). The end-to-end cascade reaches within-one-level accuracy of $0.95$
in-distribution and $0.92$ across unseen loads, with calibrated detection (expected
calibration error $=0.021$), graceful noise degradation, and $100\%$ onset detection. A
1-D residual network baseline is outperformed by the interpretable feature-based model under
limited data. The framework is fully reproducible and openly released.
\end{abstract}
...
scores substantially overstate cross-load generalization.
\item A \textbf{physics-feature analysis} (SHAP attribution plus a Fisher-versus-load-drift
study) showing that detection and faulted-phase identification are load-invariant whereas
absolute severity is load-dependent, and a \textbf{load-aware severity reformulation} that
recovers cross-load within-one-level accuracy from $0.73$ to $0.95$.
\item A complete, openly released \textbf{hierarchical cascade} integrating negative-sequence
fault-onset detection, calibrated detection, ordinal severity estimation, and faulted-phase
identification, benchmarked honestly---including against a 1-D residual network---under noise
and with conformal severity intervals.
\end{enumerate}
...
The principal limitation is that the reported results are from high-fidelity simulation; although
the negative-sequence physics that the model relies on is machine-independent, experimental
validation is required, and our per-recording normalization currently assumes the availability of
a healthy reference segment. To address these points and to advance the method beyond an
application of existing tools, we are pursuing five extensions that the recent literature
identifies as genuinely open for three-phase stator current:

\emph{(i) Sequence-component Stockwell tensor.} Rather than imaging raw phase currents, we apply
the Stockwell transform to the positive-, negative-, and zero-sequence streams and stack the three
time-frequency maps as a tensor input. The negative-sequence channel carries the load-invariant
inter-turn signature, and the frequency-dependent resolution of the Stockwell transform tracks the
$2f_0$ EPVA line and slip sidebands better than a fixed-window spectrogram.

\emph{(ii) Load-invariance-regularized feature selection.} We extend the
minimum-redundancy--maximum-relevance criterion with a term that penalizes features whose
class-conditional statistics drift across load, $J(f)=\mathrm{Rel}(f)-\lambda\,\mathrm{Red}(f)
-\gamma\,\mathrm{LoadVar}(f)$, yielding a sparse, physically traceable, load-stable subset---built
into the selection criterion rather than relegated to an opaque adversarial back-end.

\emph{(iii) Phase-coupled, operating-point-conditioned network.} A linear-time selective
state-space backbone~\cite{gu2023} processes the current; a small inter-phase coupling module
encodes the asymmetry that defines the fault; feature-wise modulation~\cite{perez2018} injects the
\emph{measured} operating point (so the model adapts to load instead of erasing it); and an
interpretable Kolmogorov--Arnold head~\cite{liu2025} regresses severity, with a physics-informed
residual tying it to the sequence-component relation.

\emph{(iv) Physics-anchored domain adaptation.} Training on simulation and adapting to experiment
with alignment anchored on physics invariants (the negative-sequence ratio and unbalance
coefficients), validated on the public three-phase ITSC current dataset~\cite{ibarram} and the
current-plus-flux dataset~\cite{cunha2021}, and across machines.

\emph{(v) Physics-constrained generative augmentation.} A conditional generator of three-phase
current that enforces Kirchhoff current balance as a hard constraint and the
negative-sequence--severity relation as a soft constraint, enabling synthesis of unseen
load$\times$severity cells---unlike per-phase augmentation, which would fabricate a spurious
imbalance.

We further plan to simulate supply voltage unbalance---the classic confounder that mimics the
inter-turn signature---to verify rejection via the voltage-sequence features, to add
group-conditional (Mondrian) conformal calibration of the severity intervals, and to broaden the
benchmark to a dozen-plus classical and deep models under the same leakage-safe protocol.
